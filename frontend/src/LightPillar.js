...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      // Fractional Brownian Motion for more organic patterns
      ...
      void main() {
        ...
        // Create darker blue and red light pillars with strong contrast
        for(float i = 0.0; i < 100.0; i++) {
          ...
          float fieldDistance = length(cosinePair) - 0.25; // Wider field for more separation
          
          // Radial boundary constraint
          float radialBound = length(pos.xz) - uPillarWidth;
          fieldDistance = blendMax(radialBound, fieldDistance, 1.0);
          fieldDistance = abs(fieldDistance) * 0.2 + 0.02; // Increased for more darkness

          // Create darker blue and red regions based on horizontal position
          vec3 gradient;
          if(pos.x < 0.0) {
            // Left side - darker blue pillar
            gradient = mix(vec3(0.0, 0.1, 0.4), vec3(0.0, 0.2, 0.6), smoothstep(15.0, -15.0, pos.y));
          } else {
            // Right side - darker red pillar  
            gradient = mix(vec3(0.4, 0.05, 0.1), vec3(0.6, 0.1, 0.2), smoothstep(15.0, -15.0, pos.y));
          }
          
          // Reduce intensity for darker appearance
          color += gradient * pow(1.0 / fieldDistance, 0.8);
        ...
        color = tanh(color * uGlowAmount * 0.5 / widthNormalization); // Reduced glow
        
        // Add darkness between pillars
        color *= 0.6; // Overall darkening
